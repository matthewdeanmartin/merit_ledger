# Security Policy for `merit_ledger`
...
1. **GitHub private vulnerability reporting** —
   [submit a confidential report](https://github.com/matthewdeanmartin/merit_ledger/security/advisories/new)
...
For maximum isolation, run `merit_ledger` inside a container so it only has access to what you
explicitly mount.
...
```bash
docker build -t merit_ledger .

# Mount the current directory into /data and pass it as an argument
docker run --rm -v "$(pwd):/data" merit_ledger /data
```
